...
| `FileDetailSidebar.vue` | ~904 | `file: FileEntry \| null, widthPercent?, versions?: FileEntry[], allowActions?: boolean, note?: string, teleportTarget?: string, teleportDisabled?: boolean` | **普通文件侧边栏**（视频播放器提取到 VideoPlayer.vue，PDF 预览提取到 PdfPreviewSection.vue）（游戏介绍/项目素材页/任务页预览视频用）。**手动 glass**：不用 `glass-strong` 类（与 main-content 相邻会触发 backdrop-filter 兄弟冲突），手动 `background/border/box-shadow`。支持图片（ImageViewer，`aspect-ratio: 4/3` 自适应宽度）、视频（自定义播放控制条）、TXT（read_text_file）、**PSD/PSB**（`usePsdThumbnail` 800px 高清缩略图 + 「用 Photoshop 打开」按钮，**首次打开延迟 300ms 等进场动画结束**，每次请求前清 JS 缓存保证感知文件修改）、**PDF**（iframe 直接渲染，WebView2 内置 PDF 引擎）、其他（图标占位）。`open_file` 用系统关联程序打开文件。可选 `versions` prop 传入多版本列表，点击 emit `select-version` 切换播放。**版本历史卡片**：卡片式布局，左列版本标签+文件大小、右侧扩展名+打开文件夹按钮，active 状态高亮。**`allowActions=true`**：底部显示重命名/删除按钮（`.sidebar-action-btn` 公共类），内联弹窗覆盖 overlay（`.sidebar-dialog-*` 公共类），emit `rename(newName)` / `delete()` 由父页面执行 invoke + 刷新。**`teleportTarget`** prop（默认 `#content-row`，FolderBrowserDialog 传 `.fb-body`）。sidebar 过渡同时动画 `transform + width`。**宽度持久化**：localStorage `pgb1-sidebar-width`，拖拽结束时写入，初始化时读取（所有页面共享）。**笔记**：可选 `note` prop + `save-note` emit，有值时 sidebar-body 内显示 NoteEditor section（watch file/note 同步 noteText）。**页面内全屏**：图片/视频/PDF/PSD 预览区右上角全屏按钮（hover 显现，`teleportDisabled` 时隐藏）；`isFullscreen` 状态切换时 JS 测量 `.main-content` 左偏移作为 `left` 值，`position: absolute; inset 其余三边: 0` 覆盖主内容区；全屏时隐藏 header/section/actions，sidebar-body `display: contents` 让预览直接参与 flex 布局；视频 `object-fit: contain` + `flex: 1`；FLIP 动画（记录起始 rect → translate+scale 逆变换 → CSS transition 播放）；Esc 退出 |
...
| `commands/attendance.rs` | ~1167 | **考勤命令**：load/save_attendance_config, save/load_attendance_password, execute_clock_action, show/close_clock_webview, open_daily_report（含预热+滚动）, test_reminder, load/save_attendance_record, schedule_overtime_reminder, show_overtime_dialog, reschedule_attendance。含 `DAILY_REPORT_INIT_JS` 常量 + `spawn_daily_report_scroll` + `send_ctrl_end` / `scroll_to_bottom_via_wheel`（Win32 API） |

...
| src/views/ | 13 | ~8910 | 页面（TaskPage 大幅瘦身 2114→1887） |
...
| 文件 | 行数 | 关键导出 | 职责 |
|------|------|---------|------|
| `useNavigation.ts` | ~74 | `setNavigation()`, `goBack()`, `routeDirection`, `setRouteDirection()` | 导航状态管理（模块级单例），驱动 TitleBar。NavAction 支持 `onLongPress(btnRect: DOMRect)`（长按回调，接收按钮位置用于锚定下拉面板）、`active`（强调样式）、`variant`（`'default'\|'success'`，控制按钮颜色变体，success = 绿色玻璃）。路由方向状态（`routeDirection` ref + `setRouteDirection()` 方法），供 TitleBar 和 MainLayout 消费实现方向感知动画 |

...
> 最后更新: 2026-04-15（Sprint 3 阶段 C 收官）
...
| src/composables/ | 22 | ~2890 | 逻辑组件（Sprint 3 阶段 C·Y-2b 新增 usePreviewVideos + useMaterialSidebar 从 TaskPage 抽离；Y-20 audit 在 useStatusBar 顶部补架构决策注释） |
| src/views/ | 18 | ~8700 | 页面（Sprint 3 阶段 C·Y-2a 将 SettingsPage 按 Tab 拆 5 个子组件到 src/views/settings/；Y-2b 将 TaskPage 的预览视频 + 素材侧边栏逻辑抽成两个 composable） |
...
| `src/config/onboarding.ts` | ~81 | 引导数据 SSOT：`PageIntro`/`GuideAnnotation` 接口，`PAGE_INTROS`（9 页介绍，仅被 PageGuideOverlay 的更多菜单消费），`PAGE_GUIDE_ANNOTATIONS`（各页面批注坐标，含 `settingsAttendance` 出勤引导专用批注） |
| `src/config/fileTypes.ts` | 8 | 文件扩展名常量 SSOT：`IMAGE_EXTS`, `IMAGE_EXTS_BROWSE`, `VIDEO_EXTS`, `TEXT_EXTS`, `PSD_EXTS`, `PDF_EXTS`（从多组件/页面提取统一） |
...
| `NormalCard.vue` | ~290 | `file: FileEntry, hasNote?, notePreview?` | 普通文件卡片（游戏介绍/项目素材页用）。视频文件 onMounted canvas 截帧；PSD/PSB 文件**两级缩略图策略**：① `file.thumbnail_path` 有值（scan 时缓存命中）→ `loading="lazy" decoding="async"` 直接渲染，与 PNG/JPG 完全等价；② 无缓存（首次或文件已修改）→ `IntersectionObserver` 进视口后触发 IPC（先清 JS 缓存防旧图污染），降级为 PS 图标直到加载完成；PDF 文件显示红色 PDF 图标；multiSelect?/checked? props + data-path + card-checkbox-shared 多选三件套。**手动 glass-subtle**（不用 backdrop-filter，避免大量卡片创建独立合成层）。**笔记**：同 MaterialCard，`hasNote`/`notePreview` props + `.note-icon` + NoteTooltip |
...
| `SidebarShell.vue` | ~371 | `show, title, widthPercent?, teleportTarget?, teleportDisabled?, hasActions?` | **侧边栏外壳组件**（从 FileDetailSidebar + TaskPage 提取）。封装容器、拖拽调宽（localStorage 持久化）、全屏 FLIP 动画（Esc 退出）、进出场动画。三个 slot：`#default`（scoped: isFullscreen + toggleFullscreen）、`#actions`（底部操作按钮）、`#overlay`（弹窗覆盖层）。手动 glass-strong（避免 backdrop-filter 兄弟冲突）。通用信息展示样式（sidebar-section / info-list / version-card）内置 |
| `FileDetailSidebar.vue` | ~904 | `file: FileEntry \| null, widthPercent?, versions?: FileEntry[], allowActions?: boolean, note?: string, teleportTarget?: string, teleportDisabled?: boolean` | 使用 SidebarShell 作为外壳。**普通文件侧边栏**（视频播放器提取到 VideoPlayer.vue，PDF 预览提取到 PdfPreviewSection.vue）（游戏介绍/项目素材页/任务页预览视频用）。支持图片（ImageViewer，`aspect-ratio: 4/3` 自适应宽度）、视频（自定义播放控制条）、TXT（read_text_file）、**PSD/PSB**（`usePsdThumbnail` 800px 高清缩略图 + 「用 Photoshop 打开」按钮，**首次打开延迟 300ms 等进场动画结束**，每次请求前清 JS 缓存保证感知文件修改）、**PDF**（iframe 直接渲染，WebView2 内置 PDF 引擎）、其他（图标占位）。`open_file` 用系统关联程序打开文件。可选 `versions` prop 传入多版本列表，点击 emit `select-version` 切换播放。**版本历史卡片**：卡片式布局，左列版本标签+文件大小、右侧扩展名+打开文件夹按钮，active 状态高亮。**`allowActions=true`**：底部显示重命名/删除按钮（`.sidebar-action-btn` 公共类），内联弹窗覆盖 overlay（`.sidebar-dialog-*` 公共类），emit `rename(newName)` / `delete()` 由父页面执行 invoke + 刷新。**`teleportTarget`** prop（默认 `#content-row`，FolderBrowserDialog 传 `.fb-body`）。**笔记**：可选 `note` prop + `save-note` emit，有值时 sidebar-body 内显示 NoteEditor section（watch file/note 同步 noteText） |
...
| `NoteDialog.vue` | ~164 | `show, title, note` (emits: save, update, cancel) | **笔记弹窗（双模式适配）**。内嵌 NoteEditor（`:save-on-blur="false"` 防止失焦关闭），渲染模式底部仅「关闭」按钮，编辑模式底部「保存+取消」。**双事件模型**：`save` = 显式保存（关闭弹窗），`update` = checkbox 切换静默保存（不关闭弹窗）。Teleport to body，`<Transition name="dialog">`。overlay `@mousedown.self.prevent` 防止点击外部关闭 |
| `PinboardDialog.vue` | ~618 | — | **[废弃]** 原贴图板弹窗外壳，已被 PinboardPage（独立窗口）替代。无引用，待删除 |
| `PinboardCanvas.vue` | ~569 | `pins, viewport, activeTool, activeColor, strokeSize, fontSize, canvasAnnotations` (emits: select-pin, deselect, update-pin, delete-pin, add-annotation, remove-annotation, add-canvas-annotation, remove-canvas-annotation, update-viewport) | **贴图板自由画布**。可平移（中键/空格+左键拖拽）+ 滚轮缩放（MAX_ZOOM=1.0，不超过 100%）。渲染 PinItem 子组件（透传 strokeSize/fontSize）。**画布标注层**：`<canvas>` overlay，支持画笔/箭头/矩形/椭圆/文字/橡皮擦直接在画布背景绘制。世界坐标系：`toWorldCoords(e)` 屏幕→世界坐标转换，`ctx.setTransform(zoom, 0, 0, zoom, panX, panY)` viewport 变换渲染。**PS 橡皮擦**：`globalCompositeOperation = 'destination-out'` 像素级擦除（3 pass 多次绘制消除抗锯齿残留）。**笔刷大小**：`strokeSize` prop 控制画笔/橡皮擦/形状工具线宽，`fontSize` prop 控制文字标注字号。**笔刷光标指示器**：跟随鼠标的圆形 div，大小 = strokeSize × zoom（仅 pen/eraser）。ResizeObserver 同步 canvas 尺寸 |
| `PinItem.vue` | ~639 | `pin, imageUrl, isSelected, activeTool, activeColor, canvasZoom, strokeSize, fontSize` (emits: select, update-position, update-size, delete, add-annotation, remove-annotation) | **单张贴图组件**。可拖拽移动 + 8 方向 resize 手柄缩放（角拖拽保持宽高比）。**Pin 级标注**：内嵌 `<canvas>` overlay，支持画笔/箭头/矩形/椭圆/文字/橡皮擦（标注坐标归一化为 0-1，与 pin 尺寸无关）。**笔刷大小**：`strokeSize` prop 控制所有绘制工具线宽，`fontSize` prop 控制文字标注字号和输入框字号。**鼠标事件**：绘制 mousemove/mouseup 绑定 window（拖出贴图边界仍可继续绘制）。**PS 橡皮擦**：同画布，`destination-out` 像素级擦除（3 pass 多次绘制）。**笔刷光标指示器**：pen/eraser 时显示跟随鼠标的圆形 div，大小 = strokeSize × min(w,h)/500，隐藏默认光标。删除按钮 28×28（z-index: 20），resize 手柄 8px（z-index: 20）。标注画布 z-index: 1 |
| `VideoPlayer.vue` | ~270 | `src: string, isFullscreen: boolean` | **视频播放器**（从 FileDetailSidebar 提取）。自定义播放控制条：播放/暂停按钮 + 进度条 + 时间显示，`object-fit: contain` + `flex: 1` 全屏适配 |
| `PdfPreviewSection.vue` | ~235 | `filePath: string, isFullscreen: boolean` | **PDF 预览+翻译**（从 FileDetailSidebar 提取）。iframe 直接渲染（WebView2 内置 PDF 引擎）+ usePdfTranslate 集成翻译 UI（进度/切换/重试） |
| `SubtaskDialog.vue` | ~251 | `show, enabledSubtasks, completedSubtasks, autoPrompt, revertPrompt, hasChanges` | **子任务管理弹窗**（从 TaskPage 提取）。子任务启用/完成状态切换，自动提醒/恢复确认，变更预览 |
...
| 文件 | 行数 | 关键导出 | 职责 |
|------|------|---------|------|
| `useNavigation.ts` | ~75 | `setNavigation()`, `goBack()`, `routeDirection`, `setRouteDirection()` | 导航状态管理（模块级单例），驱动 TitleBar。NavAction 支持 `onLongPress(btnRect: DOMRect)`（长按回调，接收按钮位置用于锚定下拉面板）、`active`（全亮强调：蓝色背景+描边）、**`hint`（弱强调：只描边不填色，与 active 互斥，用于"有活但非优先下一步"的按钮）**、`variant`（`'default'\|'success'`，控制按钮颜色变体，success = 绿色玻璃）。路由方向状态（`routeDirection` ref + `setRouteDirection()` 方法），供 TitleBar 和 MainLayout 消费实现方向感知动画 |
...
| `useNotes.ts` | ~168 | `useNotes(dirPath)`, `usePageNote(getNote, saveNote, noteKey)`, `stripMarkdown()`, `toggleCheckbox()` | **笔记系统 composable**。`NOTE_PREVIEW_LIMIT = 39`。返回 `{ notes, loading, loadNotes, getNote, hasNote, hoverPreview, previewProgress, saveNote }`。`saveNote` 乐观更新+失败回滚。接受 `Ref<string>` 或 `string` 参数。**新增 `usePageNote`**：页面级笔记快捷封装（getNote/saveNote/noteKey → pageNote/isExpanded/savePageNote）。**导出函数**：`stripMarkdown(text)` 剥离 `[text](url)` 命名链接（→text）、`**`/`*`/checklist 前缀（保留裸 URL）；`toggleCheckbox(text, lineIndex)` 切换指定行 `[ ]` ↔ `[x]`；`hoverPreview` 内部已集成 stripMarkdown |
| `useMaterials.ts` | ~45 | `loadMaterials(taskPath)` | 调用 `scan_materials` → `materials[]`。MaterialInfo 接口含 scales（比例列表）、fps（序列帧帧率，转换前 null） |
| `useDirectoryFiles.ts` | 42 | `loadFiles()`, `openInExplorer()` | 通用目录扫描 + 打开文件管理器。`FileEntry` 含可选 `thumbnail_path?: string`（PSD/PSB 磁盘缓存命中时由 scan_directory 填入） |
...
| `useStatusBar.ts` | ~504 | `useStatusBar()`, `saveConfig()`, `reloadConfig()` | 状态栏数据单例。分钟级 tick，从 `load_attendance_config` 读上下班+午休时间。**节假日**：`CalendarRegion`（auto/CN/JP/none），auto 模式通过 Rust 后端代理 `fetch_ip_country` 检测 IP（7天缓存，Sprint 3·Y-13），CN 走 Rust 代理 `fetch_cn_holiday_type`（timor.tech，含调休概念），其他国家走 Rust 代理 `fetch_nager_holidays`（date.nager.at，按年缓存）；标签简短：`休息日 🎉`/`调休`/`明天休 🎉`。**打卡状态感知**：每分钟 tick 调 `load_attendance_record`，`hasClockIn`/`hasClockOut` 驱动胶囊显隐（未打卡不显示，打下班卡触发"下班咯"）。**加班状态**：`isOvertime`（localStorage `pgb1-overtime-active` 持久化），`startOvertime()`/`endOvertime()` 管理加班中标记，供 HomePage 动态显示「结束加班」按钮。**番茄钟**：5 阶段状态机（idle→work→work-done→break→break-done→idle），秒级倒计时，归零发系统通知。**单例生命周期决策**（Sprint 3·Y-20 audit）：顶部补 8 行架构决策注释说明为什么保留 refCount 模式而非重构 provide/inject——StatusBar.vue 嵌在 TitleBar 无 v-if 导致 refCount ≥ 1 恒成立 + Y-3 已修 pendingAlignTimeout 跟踪 + 对齐 timeout 回调 refCount===0 short-circuit 三重保险。配置项（localStorage）：`showPomodoro`/`pomodoroWork`（25m）/`pomodoroBreak`（5m）/`calendarRegion`（auto）。暴露：`timeStr`/`dateStr`/`holidayLabel`/`hasClockIn`/`hasClockOut`/`workedMinutes`/`countdownMinutes`/`isLunch`/`toLunchMinutes`/`lunchLeftMinutes`/`formatMinutes`/`pomodoroPhase`/`pomodoroDisplay`/`onPomodoroClick`/`reloadHoliday`/`isOvertime`/`startOvertime`/`endOvertime` |
...
| `usePreviewVideos.ts` | ~180 | `usePreviewVideos({taskFolderPathRef, nextcloudPreviewPathRef, onAfterUpload?})` | **预览视频（03_preview）composable**（Sprint 3·Y-2b 从 TaskPage.vue 抽出）。封装 `PreviewVideoGroup` 类型、previewGroups/videoThumbnails/selectedPreviewVideo/selectedPreviewGroup/showPreviewUploadConfirm/draggedPreviewFile 六个 state；previewGroupKey/extractVersion/groupPreviewVideos/captureGroupThumbnails 四个纯函数；loadPreviewGroups/clearSelection/confirmPreviewUpload/cancelPreviewUpload 四个方法；selectedPreviewVideoAsFileEntry/selectedPreviewGroupVersionsAsFileEntries 两个 FileEntry 适配 computed。**刻意不包含**依赖 useMultiSelect 的 selectPreviewVideo / onPreviewVideoMouseDown，避免与 useMultiSelect.allPaths 形成循环依赖——这两个函数作为薄 wrapper 留在父组件 TaskPage |
| `useMaterialSidebar.ts` | ~262 | `useMaterialSidebar({taskFolderPathRef, scrollRef, materials, getNote, saveNote, refresh, onPreviewSelectionCleared})` | **素材侧边栏 composable**（Sprint 3·Y-2b 从 TaskPage.vue 抽出）。封装 selectedMaterial/versions/sidebarDialog/renameInput/sidebarNoteText/editingFps/fpsInput 七个 state；selectMaterial/closeSidebar/onSidebarNoteSave/openRenameDialog/openDeleteDialog/closeSidebarDialog/confirmRename/confirmDelete/startEditFps/cancelEditFps/confirmEditFps/openTpsFile 十二个方法；watch(selectedMaterial) → sidebarNoteText 自动同步。**preserveCardPosition**：action 前后读取卡片屏幕 Y 坐标计算 delta，补偿 scroll-content.scrollTop 消除卡片位置跳变；**参考点用 data-path 精确锁定**（原版用 `.material-card.selected` 在 wasOpen 切换时会查到两张不同卡片导致 scrollTop 被拉飞，closeSidebar 时 .selected 消失导致补偿被跳过）。跨 composable 互斥通过 onPreviewSelectionCleared 回调闭包延迟求值 |
...
| `TaskPage.vue` | ~1472 | **高** | 素材浏览主页面。**Sprint 3·Y-2b 重构**：预览视频 state/纯函数/单文件上传 → `usePreviewVideos.ts`；素材侧边栏 state/方法/preserveCardPosition → `useMaterialSidebar.ts`；父组件只保留依赖 useMultiSelect 的 selectPreviewVideo/onPreviewVideoMouseDown wrapper + 混合拖拽 performDrag + 上传/规范化/子任务/刷新/导航。**路径 SSOT**：taskFolderPathRef/nextcloudPathRef/nextcloudPreviewPathRef 三个 Ref 统一供 script/template/composable 消费。**Loading overlay**：loading 状态从 `<p v-if="loading">` 替换改成 main-content 顶部 absolute 浮标 + `<Transition name="fade">`，避免 refresh 时 v-if 卸载 tree-view 导致 scroll-content.scrollHeight 收缩 + 浏览器钳制 scrollTop 到 0，所有 refresh 场景（帧率编辑 / 重命名 / 删除 / 上传 / 窗口切换 / 手动刷新）的滚动位置全部保持。**树形视图分组**：普通任务按缩放比例分组（原始/[100]/[70]/[50]）；**Prototype 任务两级分组**：先按子分类（symbol/big_win/…），再按缩放比例子分组（原始/[100]/…），均用 section-label/group-label 渲染。名称视图平铺。**Phase 5a**：多选+拖拽上传+nextcloud 复制。**Phase 5b**：规范化。**Phase 5c**：缩放。**Phase 5d**：格式转换。**侧边栏**（使用 SidebarShell）：通用（重命名/删除）；序列帧专属：帧率行内联编辑 + [修改] 按钮；底部 `.sidebar-action-btn` 无 backdrop-filter。**sidebar-dialog**（重命名/删除弹窗）：手动 glass-strong（在 Teleport 到 #content-row 的侧边栏内，与 main-content 同层）。**03_preview 预览视频区块**：页面底部，按 baseName 分组（去 _01/_02 版本号后缀），每组一张卡片，截帧缩略图+上传状态标签（已上传/待更新/未上传）+版本数，点击打开 FileDetailSidebar（版本列表可切换），拖拽导出最新版，拖拽后弹确认弹窗复制到 nextcloud/preview/（breakdown 到 preview/breakdown/）。sidebar 过渡同时动画 `transform + width` 消除主内容区突变；useRubberBandSelect 集成（isEnabled=isMultiSelect）。**预览视频多选全链路**：选中 key = 最新版本 path（`previewGroupKey(g)`），`useMultiSelect.allPaths` 合并素材 path + 预览视频 key，`cardSelector` 扩展为 `.material-card[data-path], .preview-video-card[data-path]`；`selectPreviewVideo` 多选模式 `toggleSelection`；`onPreviewVideoMouseDown` 多选模式走统一 `performDrag(materials, previewGroups)`；`performDrag` 签名扩展接收 previewGroupsToDrag，内部合并素材文件路径 + 预览视频最新版 path；`confirmUpload` 分流：素材批量 `copy_to_nextcloud`，预览视频循环调 `copy_preview_to_nextcloud`；`preview-video-card` 加 `data-path` + `.pv-card-checkbox` + `.multi-checked` 类（position:relative）。**三按钮双级高亮**：规范化/缩放/转换三个 action 按工作流优先级（规范化 > 缩放 > 转换）选出 primary 全亮（`active`），其余有活按钮仅描边（`hint`）；`hasNormalizeWork` ref（异步 `preview_normalize` 检测，loadMaterials 后触发）+ `hasScaleWork`/`hasConvertWork` computed（与 ScalePage/ConvertPage 判定口径完全一致）；`watch([三 flag])` 变化时重刷 updateNavigation。**笔记系统**：MaterialCard 传入 `:has-note`/`:note-preview`（从 useNotes 取），自定义侧边栏内嵌 NoteEditor（watch selectedMaterial 切换），sub-title-bar 旁 `.note-btn` 打开 NoteDialog 编辑任务卡片笔记（`card:{taskId}` key，与 ProjectPage TaskCard 共用），双 useNotes（taskFolderPathRef 素材笔记 + projectPathRef 页面笔记） |
...
| `ConvertPage.vue` | ~746 | **中** | **格式转换执行页面**（Phase 5d）。控制面板 Teleport 到 #content-row，手动 glass-medium（无 backdrop-filter，与 main-content 同层兄弟）。静帧默认全选，序列帧需手动标注 FPS 才算「已注释」。**TP 预设折叠面板**：侧边栏"开始制作"按钮上方，可展开收起，包含 Scale（f64）和 WebP Quality（u32）输入框，失焦时保存到全局设置。invoke `start_conversion` 时传 `tp_scale` / `tp_webp_quality`。监听 `sequence-conversion-failed` / `conversion-organized` 事件；useRubberBandSelect（isEnabled=ref(true)，始终开启）。**进度去重修复**（v2.5.2）：`expectedNames` 白名单 + `organizedNames` Set 去重，只统计当前会话选中素材且每名只计一次；`handleStart` 先调 `stop_conversion` 清理旧会话防事件泄漏；`onUnmounted` 调 `stop_conversion` 防残留 watcher。**跳过序列帧总数收敛修复**：`sequence-conversion-failed` 监听器新增 `failedNames` Set（对称于 `organizedNames`），跳过的序列帧（TP 未点发布）从 `conversionProgress.total` 剔除，保证 `current/total` 能收敛到相等，解锁「完成转换」按钮 |
...
| `SettingsPage.vue` | ~590 | 中 | **全局设置页面父组件**（Sprint 3·Y-2a 从 1038 → 590 行，-43%）。只负责 Tab 导航 / editSettings 共享状态 / 保存闭环 / 5 个子组件的 v-show 切换 + sidebar-footer 保存按钮（三 Tab 共用 handleSave，attendance 通过 attendanceRef.value.save() 调子组件自包含的 save）。**5 个子组件**在 `src/views/settings/` 子目录（见下表）。**数据流分层**：workflow/translation/general 用 `v-model="settingsModel"`（writable computed 包住 editSettings 避免 null 问题，defineModel 在子组件侧深度修改 editSettings 触发父 watch(deep) 设 isDirty）；attendance 用 `ref="attendanceRef"` + `defineExpose({save, isDirty, isSaving, saved})` 完全自包含；about 无 props 内部消费 useUpdater()。**样式穿透**：父组件 `<style scoped>` 的共享 form 样式（`.settings-section`/`.form-group`/`.form-input` 等）全部用 `:deep()` 穿透到子组件 scoped 边界，避免 5 次重复。**v-show 而非 v-if**：避免切 Tab 时 AttendanceSettings 卸载丢失未保存字段 + 消除首次进入 attendance 的冷启动延迟。**语言切换副作用**：子组件只 `locale.value = val` + saveSettings + `emit('persisted')`，父组件 `watch(language)` 重新调 setNavigation 让 `t('settings.title')` 用新 locale 重求值。**出勤引导**：`route.query.guide === 'attendance'` 时自动弹出 `settingsAttendance` 专属批注。 |
| `settings/AboutSettings.vue` | ~32 | 低 | 设置页·关于 Tab 子组件。纯展示：APP_NAME/APP_VERSION/APP_DEVELOPER + useUpdater 的检查更新按钮。无 props |
| `settings/WorkflowSettings.vue` | ~45 | 低 | 设置页·工作流 Tab 子组件。`defineModel<AppSettings>` + pickFile，三个路径字段（imaginePath / texturePackerCliPath / texturePackerGuiPath）browse 按钮 |
| `settings/TranslationSettings.vue` | ~56 | 低 | 设置页·翻译 Tab 子组件。`defineModel<AppSettings>` 纯表单：API key / 模型下拉（gemini-2.5-flash/flash-lite/3-flash-preview/2.5-pro）/ 全局快捷键 / 计算器键开关 / 默认语言对（中/英/日） |
| `settings/GeneralSettings.vue` | ~121 | 低 | 设置页·通用 Tab 子组件。`defineModel<AppSettings>` + useTheme + useScale + useI18n。项目根目录 / 语言切换 / 主题切换 / 默认帧率 / 透明背景 / 开机自启 / UI 缩放档位（75%~150%）。**语言/缩放是运行时偏好，立即持久化**——onLanguageChange / onScaleChange 直接 await saveSettings(settings.value) 后 emit('persisted') 让父组件重置 isDirty |
| `settings/AttendanceSettings.vue` | ~293 | 中 | 设置页·日报打卡 Tab 子组件。**完全自包含**：18 个出勤专属 state + init() 加载 load_attendance_config + save_attendance_password + save() 保存（含 trim + 邮箱校验 + reschedule_attendance）+ handleTestClock / handleTestDailyReminder + onMounted 监听 clock-test-progress 事件（onUnmounted 清理）+ 2 秒 saved 高亮。**defineExpose**({save, isDirty, isSaving, saved}) 给父组件 sidebar-footer 的保存按钮使用（UI 1:1 保持在左下角）。mode 三段切换（off/auto/record_only）+ 四档时间 input + 账号密码（show/hide）+ 测试连接按钮 |
| `ReminderPage.vue` | ~448 | 中 | **日报打卡提醒弹窗**，支持 clock-in/clock-out/daily-report/overtime 四种类型。**加班流程**：clock-out 弹窗点「加班」→ emit `overtime-started` 事件 + 关窗（不再弹出加班时间设置窗口），主页显示「结束加班」按钮。**漏打退勤检测**：clock-in 弹窗 onMounted 时检查 localStorage 加班残留或 attendance record 中 `last_clock_in > last_clock_out`，命中则显示橙色警告条「昨天忘记打退勤卡了」+ emit `overtime-ended` 清除加班按钮 |
| `OvertimePage.vue` | ~140 | 低 | **加班时间设置弹窗**（已弃用，保留代码未删除。快捷按钮 +30分/+1小时/+2小时 + 自定义输入） |
...
| `design-system.css` | ~1466 | **SSOT**：颜色（冷科技蓝色板+冷蓝灰中性色）、间距、排版（URW DIN + 更纱黑体）、圆角（工业风收窄）、卡片、标签、过渡、毛玻璃变量。暗色主题 v2.0 冷色工业终端风格。`.section-label`+`.group-label` 标题样式。**新增 token**：`--overlay-backdrop`（弹窗遮罩）、`--canvas-bg`（Canvas 背景）。弱化 Material 阴影，改用透明度+冷蓝边框拉层级。**Hover SSOT**：`--shadow-card-hover` 含 ring 光晕（`0 0 0 1px rgba(100,180,255,0.30)`）覆盖所有卡片；`--card-hover-lift` = -3px。**优先度 token**：菜单胶囊用 `--priority-{h/m/l}-{bg/text/active}`（半透明）；卡片圆点用 `--priority-{high/medium/low}-dot`（= color-danger/warning/success 纯实色）。**深色模式 `--text-tertiary`**：`#6B6E77`（原 `#4A4D54` 对比度仅 2.2:1，已提升至 3.8:1）。**TransitionGroup FLIP**：`.card-move { transition: transform ... }` 使所有 `name="card"` 分组在排序时平滑位移。**公共类 `.folder-btn`**：28×28 图标按钮，透明背景，hover 蓝色 wash。**公共类 `.view-btn`/`.sort-tab`**：视图切换/排序标签页公共样式。**公共类 `.control-panel`**：控制面板公共类（ScalePage/ConvertPage 共用）。**公共类 `.sidebar-actions`/`.sidebar-action-btn`**：侧边栏底部悬浮操作按钮（TaskPage + FileDetailSidebar 共用），`.danger` 变体 hover 红色。**公共类 `.sidebar-dialog-overlay`/`.sidebar-dialog`/`.sidebar-dialog-*`**：侧边栏内联弹窗。**笔记渲染公共类**：`.note-rendered`（渲染视图容器，链接/checkbox/粗体/斜体样式）、`.note-toolbar`/`.note-toolbar-btn`（工具栏 4 按钮）、`.note-edit-btn`（渲染视图右上角编辑按钮） |
...
| `dialog.css` | ~120 | **弹窗公共样式**（从多个 Dialog 组件提取统一）。`.dialog-overlay`（遮罩 + 居中布局）、`.dialog-content`（玻璃面板）、`.dialog-title`/`.dialog-body`/`.dialog-actions`（结构）、`.dialog-btn`（按钮变体：primary/danger/ghost）、进出场动画（translateY/scale/opacity） |
...
| 文件 | 行数 | 关键导出 | 职责 |
|------|------|---------|------|
| `src/utils/format.ts` | ~23 | `formatSize(bytes)`, `normalizeDeadline(raw)` | 通用格式化工具（从 MaterialCard/NormalCard/TaskCard/多页面提取统一，消除重复实现） |
| `src/types/task.ts` | ~27 | `GlobalTaskChild`, `GlobalTask`, `GlobalTaskConfig`, `ApplyTaskResult`, `ArchivedVersion` | 任务管理 TypeScript 类型定义（从 TaskListPage 等页面提取） |
| `src/types/material.ts` | ~19 | `PreviewVideoEntry`, `MaterialVersion` | 素材相关 TypeScript 类型定义（从 TaskPage 等页面提取） |
...
|------|------|------|
| `main.rs` | 6 | 应用入口 |
| `lib.rs` | ~312 | Tauri 初始化、命令注册（70 个）、插件注册（opener/drag/dialog/clipboard/notification/**autostart**）、Windows Acrylic 毛玻璃、调度器初始化 + 补打检测、hotkey 全局快捷键初始化、**启动时同步 autolaunch 状态** |
| `models.rs` | ~635 | 数据模型（29 个 struct + 3 个 enum，含贴图板 PinAnnotation/PinInfo/PinboardViewport/PinboardCanvas/PinboardData）。ProjectConfig 新增 default_ae_file 字段。新增 PreviewVideoEntry（含 upload_status）。新增 **PreviewSettings**（default_fps/background_transparent），AppSettings 加 preview 字段。`GeneralSettings.ui_scale` 默认值 `1.0`（首次运行不再使用自动缩放）。`GeneralSettings.auto_start: bool`（开机自启，默认 false）。**WorkflowSettings** 新增 `tp_scale: f64`（默认 0.5）/ `tp_webp_quality: u32`（默认 80）。**StartConversionRequest** 新增同名字段 |
| `hotkey.rs` | ~144 | **全局快捷键**：`start_hotkey_listener`（独立线程 Win32 消息循环）、`do_toggle_window`、`parse_shortcut`。支持计算器键（0xB7） |
...
| `scan_directory` | app_handle, dir_path | Vec\<FileEntry\> | 通用一层目录扫描（目录在前，文件在后）。PSD/PSB 文件额外检查 256px 磁盘缓存是否命中，命中则填入 `FileEntry.thumbnail_path`（hash 计算与 `extract_psd_thumbnail` 完全一致：path + mtime + 256u32） |
...
| `rename_material` | task_path, base_name, new_base_name, material_type | () | 重命名素材所有版本（00_original/01_scale/**/02_done/**/nextcloud），序列帧同步重命名目录+内部帧文件。**帧文件下划线分隔符兼容**：序列帧帧文件按命名规则为 `{base_name}_{帧号}.png`（下划线分隔），`matches_base_name` 只认连字符，故 rename_material 的 sequence 分支内对帧文件单独放宽判定为 `stem == base_name \|\| stem.starts_with("{base_name}_") \|\| stem.starts_with("{base_name}-")`，外层目录判定保持严格 |
...
| `FileEntry` | 文件/目录条目 DTO。**新增** `thumbnail_path: Option<String>`（PSD/PSB 缓存路径，`skip_serializing_if = None`） |

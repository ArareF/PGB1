...
> 最后更新: 2026-03-05（PSB 缩略图支持 + PSD 磁盘缓存 + FolderBrowserDialog + 卡片性能修复）
...
| `FileDetailSidebar.vue` | ~935 | `file: FileEntry \| null, widthPercent?, versions?: FileEntry[], allowActions?: boolean, note?: string, teleportTarget?: string` | **普通文件侧边栏**（游戏介绍/项目素材页/任务页预览视频用）。**手动 glass**：不用 `glass-strong` 类（与 main-content 相邻会触发 backdrop-filter 兄弟冲突），手动 `background/border/box-shadow`。支持图片（ImageViewer）、视频（自定义播放控制条）、TXT（read_text_file）、**PSD/PSB**（`usePsdThumbnail` 800px 高清缩略图 + 「用 Photoshop 打开」按钮）、**PDF**（iframe 直接渲染，WebView2 内置 PDF 引擎）、其他（图标占位）。`open_file` 用系统关联程序打开文件。可选 `versions` prop 传入多版本列表，点击 emit `select-version` 切换播放。**版本历史卡片**：卡片式布局，左列版本标签+文件大小、右侧扩展名+打开文件夹按钮，active 状态高亮。**`allowActions=true`**：底部显示重命名/删除按钮（`.sidebar-action-btn` 公共类），内联弹窗覆盖 overlay（`.sidebar-dialog-*` 公共类），emit `rename(newName)` / `delete()` 由父页面执行 invoke + 刷新。**`teleportTarget`** prop（默认 `#content-row`，FolderBrowserDialog 传 `.fb-body`）。sidebar 过渡同时动画 `transform + width`。**笔记**：可选 `note` prop + `save-note` emit，有值时 sidebar-body 内显示 NoteEditor section（watch file/note 同步 noteText） |

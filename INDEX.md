...
| `CODE_INDEX.md` | 全量源代码文件职责索引（97 个文件，~33400 行） | 编程阶段，了解代码现状 |
...
| 打卡逻辑 | 不做工作日/假期判断，所有弹窗有跳过按钮；三档 mode（off/auto/record_only）+ 日报独立 enabled。**加班流程**：退勤弹窗点「加班」→ 主页快捷功能区显示绿色「结束加班」按钮 → 用户点击后弹出退勤打卡窗口。次日出勤弹窗检测漏打退勤并警告 | `design/界面设计.md` L874 |

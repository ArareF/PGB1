...
pub(super) fn add_yahe_font(
    ...
    let (ascent, descent, cap_height, bbox) = {
        ...
        match Face::parse(&ttf_bytes, 0) {
            ...
            // N-29 知情降级：ttf-parser 解析失败时用 msyh（微软雅黑）的典型度量兜底。
            // 触发条件极窄：TTC 提取已成功（即字体字节合法），但 ttf-parser 对该字体异常。
            // 实测几乎不发生。如果真落到这里，PDF 字形大小/基线会略偏，但不会崩溃——
            // 这些数值匹配 msyh，对大多数 CJK 字体来说视觉误差可接受。
